...
let songs = [];
// Parse UserID and User from localStorage
// Parse UserID and User from localStorage
...
if (album != null) {
    ...
    console.log("ở album");
    if (album == "playlist") {
        let likedSongList = songList.filter(e => {
...
document.querySelector(".playlist-button .like-btn").addEventListener("click", () => {
    // Check if UserID is not available, redirect to homepage
    if (!UserID) {
        location.href = "TrangChu.aspx";
        return; // Stop further execution
    }

    // Create a new array to store liked albums
    let new_LikedAlbums;

    // Check if the current album is already liked
    if (User_LikedAlbums.includes(songs[0].album)) {
        // If liked, remove it from the liked albums
        new_LikedAlbums = User_LikedAlbums.filter((album) => album !== songs[0].album);
    } else {
        // If not liked, add it to the liked albums
        new_LikedAlbums = [...User_LikedAlbums, songs[0].album];
    }

    // Update the liked albums of the current user
    currentUser.LikedAlbums = new_LikedAlbums;
...
Array.from(document.querySelectorAll(".songItem .like-btn")).forEach((likeButton, index) => {
    // Add click event listener to each like button
    likeButton.addEventListener("click", () => {
        // Check if UserID exists, if not, redirect to homepage
        if (!UserID) {
            location.href = "TrangChu.aspx";
            return; // Stop further execution
        }

        // Get the ID of the clicked song item
        let songId = songs[index]?.id;

        // Ensure songId exists before proceeding
        if (!songId) {
        ...
        if (User_LikedSongs.includes(songId)) {
            // If liked, remove it from the liked songs
            User_LikedSongs = User_LikedSongs.filter((id) => id !== songId);
        } else {
            // If not liked, add it to the liked songs
            User_LikedSongs.push(songId);
        }

        // Update the liked songs of the current user
        currentUser.LikedSongs = User_LikedSongs;
